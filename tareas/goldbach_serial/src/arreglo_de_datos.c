...
typedef struct arreglo_nodo_cola {
    arreglo_de_datos_t arreglo_sumas;
    arreglo_nodo_cola_t* next;
    int64_t position;
    int64_t value;
    int validate;
} arreglo_nodo_cola_t;
...
int insert(arreglo_de_datos_t* arreglo, int64_t value, int valid) {
    ...
    arreglo_nodo_cola_t* new_nodo = (arreglo_nodo_cola_t*) calloc(1,
                               sizeof(arreglo_nodo_cola_t));
    if (new_nodo) {
        ...
        if (arreglo -> ultimo == NULL) {
            arreglo -> primero = new_nodo;
            arreglo -> ultimo = new_nodo;
        } else {
            arreglo -> ultimo -> next = new_nodo;
            arreglo -> ultimo = arreglo -> ultimo -> next;
        }
    } else {
        error = EXIT_FAILURE;
 ...
 * @return arreglo_nodo_t* 
 */
arreglo_nodo_cola_t* search(arreglo_de_datos_t* arreglo, int64_t searching) {
    assert(arreglo);
    arreglo_nodo_cola_t* actual = arreglo -> primero;
    while (actual-> value) {
...
void arreglo_destroy(arreglo_de_datos_t* arreglo) {
    assert(arreglo);
    arreglo_nodo_cola_t* actual = arreglo->primero;
    arreglo_nodo_cola_t* borrar = NULL;
    while (actual) {
        borrar = actual -> next;
        nodo_destroy(actual);
        actual = borrar;
